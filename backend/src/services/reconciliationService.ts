...
export async function getReconciliation(database: PrismaClient, venueId?: string) {
  const rows = await database.$queryRaw<Array<{
    ...
    SELECT c.charge_id AS "chargeId", c.amount_minor AS "chargeAmount", c.currency AS "chargeCurrency",
      ...
      COUNT(r.id) FILTER (WHERE r.status = 'SUCCEEDED')::int AS "successfulRefundCount",
      COALESCE(SUM(r.amount_minor) FILTER (WHERE r.status = 'SUCCEEDED'), 0)::int AS "refundAmount",
      CASE
        ...
        WHEN b.status::text IN ('CONFIRMED', 'COMPLETED') AND COUNT(r.id) FILTER (WHERE r.status = 'SUCCEEDED') > 0 THEN 'multiple_outcomes'
        WHEN b.status::text NOT IN ('CONFIRMED', 'COMPLETED') AND COUNT(r.id) FILTER (WHERE r.status = 'SUCCEEDED') = 0 THEN 'not_confirmed_or_refunded'
        WHEN b.status::text NOT IN ('CONFIRMED', 'COMPLETED') AND COUNT(r.id) FILTER (WHERE r.status = 'SUCCEEDED') <> 1 THEN 'multiple_refunds'
        WHEN b.status::text NOT IN ('CONFIRMED', 'COMPLETED') AND COALESCE(SUM(r.amount_minor) FILTER (WHERE r.status = 'SUCCEEDED'), 0) <> c.amount_minor THEN 'incorrect_refund_amount'
        ELSE NULL
      END AS issue
